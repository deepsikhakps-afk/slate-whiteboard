...
5. **Resizing**: on window resize, the current canvas content is preserved (via `ta`/`putImageData`) before the canvas is resized and redrawn, so in-progress work isn't lost.
